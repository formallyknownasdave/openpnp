...
@Root
public class StarNozzleTipCalibration extends AbstractModelObject {
    ...
    public static class ModelBasedRunoutCompensation implements RunoutCompensation {
        ...
        public Length getRadius() {
            return new Length(radius,this.units);
        }
    ...
    @Attribute(required = false)
    private RunoutCompensationAlgorithm runoutCompensationAlgorithm =
    ...
    public RunoutCompensation generateModel(StarNozzle nozzle, Camera camera, boolean targetTip, double angleStart, double angleStop, double angleSubdivisions, int allowMisdetections, RunoutCompensationAlgorithm algorithm) throws Exception {
        ...
        if (nozzle.getNozzleTip() == null) {
            throw new Exception("StarNozzleTipCalibration requires a nozzle tip to be installed");
        }
        ReferenceNozzleTipLocator locator = (ReferenceNozzleTipLocator)(nozzle.getNozzleTip().getLocator());
        if (locator == null) {
            throw new Exception("StarNozzleTipCalibration requires a nozzle tip with a locator pipeline");
        }
        calibrating = true;
        // determine the resulting angleIncrements
        double angleIncrement = ( angleStop - angleStart ) / angleSubdivisions;
        ...
        Logger.debug("[starNozzleTipCalibration]starting measurement; angleStart: {}, angleStop: {}, angleIncrement: {}, angleSubdivisions: {}",
                angleStart, angleStop, angleIncrement, angleSubdivisions);
        ...
        for (int i = 0; i <= angleSubdivisions; i++) {
            ...
            Logger.debug("[starNozzleTipCalibration]i: {}, measureAngle: {}", i, measureAngle);
            ...
            Location expectedLocation;
            if (targetTip) {
                // For targetting the nozzle tip, we artificially create an expected run-out of 1 mm and
                // Compare the actual values to these to compute the true run-out
                expectedLocation = new Location(LengthUnit.Millimeters, 1.0, 0, 0, measureAngle).rotateXy(measureAngle);
            } else {
                // For camera calibration, the expected location is just the nozzle location
                expectedLocation = measureLocation;
            }
            // locate the nozzle tip
            NozzleTipLocator.NozzleTipAlignmentOffset tipOffset = locator.findOffsets(nozzle.getNozzleTip(), measureLocation, nozzle);

            if (tipOffset != null) {
                // for later usage in the algorithm, the measureAngle is stored to the offset location in millimeter unit
                Location offset = tipOffset.getLocation().derive(null, null, null, measureAngle);

                Logger.warn("tipOffset == ({},{})",offset.getX(),offset.getY());
                // add offset to array
                nozzleTipMeasuredLocations.add(offset);
                nozzleTipExpectedLocations.add(expectedLocation);
                ...
                Logger.warn("tipOffset == null");
                misdetects++;
                if (misdetects > allowMisdetections) {
                    calibrating = false;
                    throw new Exception("Too many vision misdetects. Check pipeline and threshold.");
                }
            }
        ...
        if (nozzleTipMeasuredLocations.size() < Math.max(3, angleSubdivisions + 1 - allowMisdetections)) {
            throw new Exception("Not enough results from vision. Check pipeline and threshold.");
        }

        // Configuration.get().getScripting().on("NozzleCalibration.Finished", params);
        RunoutCompensation model;
        switch (algorithm)
        {
            case Model:
                model = new ModelBasedRunoutCompensation(nozzleTipMeasuredLocations);
                break;
            case ModelAffine:
                model = new ModelBasedRunoutCompensation(nozzleTipMeasuredLocations, nozzleTipExpectedLocations);
                break;
            case ModelNoOffset:
                model = new ModelBasedRunoutNoOffsetCompensation(nozzleTipMeasuredLocations);
                break;
            case ModelNoOffsetAffine:
                model = new ModelBasedRunoutNoOffsetCompensation(nozzleTipMeasuredLocations, nozzleTipExpectedLocations);
                break;
            case ModelCameraOffset:
                model = new ModelBasedRunoutCameraOffsetCompensation(nozzleTipMeasuredLocations);
                break;
            case ModelCameraOffsetAffine:
                model = new ModelBasedRunoutCameraOffsetCompensation(nozzleTipMeasuredLocations, nozzleTipExpectedLocations);
                break;
            default:
                model = new TableBasedRunoutCompensation(nozzleTipMeasuredLocations);
        }
        calibrating = false;
        return model;
    }
    ...
    public void calibrate(StarNozzle nozzle) throws Exception {
        calibrate(nozzle, false, false);
    }

    public void calibrateCamera(StarNozzle nozzle) throws Exception {
     ...
     */
    public Location getCalibratedOffset(StarNozzle nozzle, double angle) {
     ...
     */
    public Location getCalibratedCameraOffset(StarNozzle nozzle, Camera camera) {
        try {
    ...
    public void reset(StarNozzle nozzle) {
    ...
    private RunoutCompensation getRunoutCompensation(StarNozzle nozzle) {
    ...
    private void setRunoutCompensation(StarNozzle nozzle, RunoutCompensation runoutCompensation) {
    ...
    public String getCalibrationInformation(StarNozzle nozzle) {
        return getRunoutCompensation(nozzle).toString();
    }

    public boolean isCalibrated(StarNozzle nozzle) {
    ...
    public boolean isRecalibrateOnNozzleTipChangeInJobNeeded(StarNozzle nozzle) {
        return recalibrationTrigger == RecalibrationTrigger.NozzleTipChangeInJob;
    }

    public boolean isRecalibrateOnNozzleTipChangeNeeded(StarNozzle nozzle) {
    ...
    public boolean isRecalibrateOnHomeNeeded(StarNozzle nozzle) {
        return recalibrationTrigger == RecalibrationTrigger.NozzleTipChange
